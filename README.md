...
2. **Name your new repository** using the convention: `Nevada Data Pipeline` (e.g., `il-data-pipeline`, `tx-data-pipeline`).
...
3. **Update the state abbreviation** in both workflow files:
   ...
   **In `.github/workflows/scrape-and-format-data.yml`:**
   ...
   ```yaml
   env:
     STATE_CODE: nv # CHANGE THIS to your state abbreviation
...
uses: windy-civi/toolkit/actions/scrape@main
with:
  state: nv # State abbreviation (required)
...
git clone https://github.com/YOUR-ORG/Nevada Data Pipeline
cd Nevada Data Pipeline
